...
def test_detector_and_reid_component_configs_load_separately():
    ...
    assert reid_cfg["url"] == "https://example.com/reid/lmbn_n_duke.pth"
...
def test_mot17_dataset_exposes_default_reid():
    ...
    assert get_benchmark_reid_cfg(cfg) == {
        ...
        "model_url": "https://example.com/reid/lmbn_n_duke.pth",
        "url": "https://example.com/reid/lmbn_n_duke.pth",
        "device": "",
        "half": True,
        "preprocess": "resize",
    }
...
def test_ensure_dataset_source_available_downloads_missing_dataset(monkeypatch):
    ...
    assert calls == {
        "runs_url": "",
        "dataset_url": "https://example.com/datasets/MMOT-OBB.zip",
